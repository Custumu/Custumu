...
export default function Navbar({
  ...
  return (
    <>
      <header className="h-16 border-b border-dark-border bg-dark-surface/90 backdrop-blur-md px-4 sm:px-6 flex items-center justify-between z-30 sticky top-0">
        {/* Brand & Document Name */}
        <div className="flex items-center space-x-3 sm:space-x-4">
          <div className="flex items-center space-x-2.5 cursor-pointer" onClick={onResetDocument} title="Custumu Home">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-slate-400 -mt-1 hidden sm:inline">custumu.com</span>
            </div>
          </div>
          ...
          {/* Active File Pill */}
          <div className="hidden md:flex items-center space-x-2 bg-dark-card/80 border border-dark-border px-3 py-1.5 rounded-full text-xs text-slate-300">
            <FileText className="w-3.5 h-3.5 text-brand-400" />
            <span className="font-medium truncate max-w-[200px]">{documentName || 'Master_Agreement.pdf'}</span>
            <span className="text-slate-500">•</span>
            <span className="text-slate-400">{pageCount} {pageCount === 1 ? 'Page' : 'Pages'}</span>
          </div>
        </div>

        {/* Center: Undo / Redo */}
        <div className="hidden lg:flex items-center space-x-1 bg-dark-card/60 p-1 rounded-lg border border-dark-border">
          <button
            onClick={onUndo}
            disabled={!canUndo}
            className={`p-1.5 rounded transition ${canUndo ? 'text-slate-300 hover:text-white hover:bg-dark-hover' : 'text-slate-600 cursor-not-allowed'}`}
            title="Undo (Ctrl+Z)"
          >
          ...
          <button
            onClick={onRedo}
            disabled={!canRedo}
            className={`p-1.5 rounded transition ${canRedo ? 'text-slate-300 hover:text-white hover:bg-dark-hover' : 'text-slate-600 cursor-not-allowed'}`}
            title="Redo (Ctrl+Y)"
          >
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center bg-dark-card border border-dark-border rounded-full p-0.5">
            <button
          ...
          <div className="relative">
            ...
            {showExportMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-dark-surface border border-dark-border rounded-xl shadow-2xl py-1.5 z-50 text-xs">
                <button
                  onClick={handleDownloadPdf}
                  className="w-full text-left px-4 py-2.5 hover:bg-dark-hover flex items-center space-x-2 text-slate-200 transition"
                >
      ...
      {showPrivacyModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-dark-surface border border-dark-border rounded-2xl max-w-md w-full p-6 shadow-2xl animate-fade-in">
            <div className="flex items-center space-x-3 mb-4">
              <div className="p-2.5 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                <ShieldCheck className="w-6 h-6" />
              </div>
              <div>
                <h3 className="text-base font-semibold text-white">Custumu Dual Privacy Engine</h3>
                <p className="text-xs text-slate-400">custumu.com enterprise guarantee</p>
              </div>
            </div>
