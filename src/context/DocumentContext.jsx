...
export function DocumentProvider({ children }) {
  ...
  const pushState = useCallback(
    (newBuffer, newMeta, newAnnotations = annotations) => {
  ...
  const handleExportWord = useCallback(() => {
    ...
    exportTextToWord(
      docName || 'Document',
      textContent,
      `Custumu_${(docName || 'document').replace(/\.pdf$/i, '')}.doc`
    );
      ...
      const tables = customTables || [
        {
          ...
          rows: [{ Item: 'Document Analysis', File: docName || 'Document', Status: 'Verified' }],
        },
      ];
    ...
    async (action) => {
      ...
      if (action.type === 'SPLIT_PDF') {
        ...
        if (
          ...
          (!action.pageNumbers?.length && !action.ranges?.length)
        ) {
          ...
          const indices = action.pageNumbers.map((p) => p - 1).filter((p) => p >= 0 && p < totalP);
          ranges = [
            {
